...
SET od.unit_price = 
    CASE 
        ...
        ELSE p.price
    END; -- Update order_details.unit_price

UPDATE order_details
SET total_price = (unit_price * quantity) * (1 - discount_applied); -- Update order_details.total_price
...
SET o.total_amount = od.total_amount; -- update orders.total_amount = order_details.total_price

UPDATE payments
SET refunded_amount = 
    CASE 
        WHEN payment_method = 'PayPal' THEN 0.01
        ELSE 0
    END; -- Update payments.refunded_amount

UPDATE payments
JOIN orders ON payments.order_id = orders.order_id
SET payments.amount = 
    CASE 
        WHEN payments.payment_method = 'PayPal' THEN (orders.total_amount + orders.shipping_cost) * (1 - payments.refunded_amount)
        ELSE orders.total_amount + orders.shipping_cost
    END; -- Update payments.amount
